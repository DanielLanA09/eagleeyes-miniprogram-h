<template>
    <div>
      <div class="g-container" :style="{height:mstyle.mH+'px',width:mstyle.mW+'px'}">
        <canvas class="g-canvas" canvas-id="radar1" :style="{height:mstyle.mH+'px',width:mstyle.mW+'px'}" @click="$emit('onCanvasClick')"></canvas>
  ...
  methods: {
    ...
    drawCircle(ctx) {
      ...
      for (var i = 0; i < me.mCount; i++) {
        ...
        ctx.setFillStyle("rgba(50, 187, 143, 0.8)");
        ctx.fill();
      }
      ctx.restore();
    }
